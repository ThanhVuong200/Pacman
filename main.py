...
class MovingEntity(Entity):
    ...
    def __init__(self, xPos, yPos, size, speed, spriteName, imgSpeed, imgPerCycle):
        ...
        self.direction = 0   # Hướng di chuyển (0=phải, 1=trái, 2=lên, 3=xuống)
        self.xSpeed = 0
        self.ySpeed = 0
        self.subImg = 0      # Chỉ số hình ảnh trong chu kỳ hoạt ảnh
        self.x_start = xPos  # Vị trí x ban đầu dùng để reset
        self.y_start = yPos  # Vị trí y ban đầu dùng để reset
    ...
    def updatePos(self):
        if not (self.xSpeed == 0 and self.ySpeed == 0):
            ...
            # Cập nhật hình ảnh phụ trong hoạt ảnh
            self.subImg += self.imgSpeed
            if self.subImg >= self.imgPerCycle:
                self.subImg = 0

        # Đảm bảo vị trí thực thể luôn nằm trong giới hạn màn hình
...
class Pacman(MovingEntity):
    ...
    def __init__(self, xPos, yPos):
        super().__init__(xPos + 8, yPos, 32, 2, "pacman1.png", 0.2, 4)
        self.lives = 3
...
class PacGum(StaticEntity):
    ...
    def draw(self):
        pygame.draw.circle(screen, pygame.Color(255, 183, 174), (self.xPos + self.size // 2 + 6, self.yPos + self.size // 2 + 8), (self.size // 2) + 1)

class SuperPacGum(StaticEntity):
    def __init__(self, xPos, yPos):
        super().__init__(xPos, yPos, 18) 
        self.flicker = 0
    ...
    def draw(self):
        if self.flicker < 30:
            pygame.draw.circle(screen, pygame.Color(255, 215, 0), (self.xPos + self.size // 2, self.yPos + self.size // 2), self.size // 2)

            pygame.draw.circle(screen, pygame.Color(255, 165, 0), (self.xPos + self.size // 2, self.yPos + self.size // 2), self.size // 2, 2)
...
class Ghost(MovingEntity):
    def __init__(self, xPos, yPos, sprite, time):
        ...
        self.ghost_frightened_1 = pygame.image.load('res/img/ghost_frightened_1.png')
...
def init():
    ...
    size = 8    # Kich' thuoc' moi~ o

    for i in range(len(level)):  # Duyệt qua từng phần tử trong level để khởi tạo đối tượng
 ...
 # Thêm Pacman và các ma vào danh sách entities
  ...
  # Phân loại các đối tượng vào danh sách wall và ghosts
    for i in entities:
...
def main():
    ...
    while running:
        ...
        if not gameOver and not win:
            ...
            drawString("Press Space To Replay",     120, 250)
        elif win:
            playMusic("win.wav")
